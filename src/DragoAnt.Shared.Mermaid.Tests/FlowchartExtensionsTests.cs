namespace DragoAnt.Shared.Mermaid.Tests;
...
public class FlowchartExtensionsTests
{
    ...
    public void CreationTest()
    {
        ...
        {
            Direction = FlowchartGraphDirection.TD
        };
        ...
        graph.AddRelation("Country_States", "CountryState_Country", "fa:fa-1 CountryId #\"{}()[]<>%~=' fa:fa-N", FlowchartRelationLineEnding.Arrow,
            FlowchartRelationLineStyle.Dots);

        graph.AddItemInteractionCallback("CountryState_Country", $"call exampleCallback(DragoAnt.Domain.CountryState.Country)",
    ...
    [Theory]
    [InlineData("""Country#"{}()[]<>%~='""", "Country#_____________")]
